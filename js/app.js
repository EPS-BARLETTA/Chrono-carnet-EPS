...
CARNET_URL="https://carnetentrainementv2.vercel.app/",
    ...
    x=>
      x.onclick=()=>{
        ...
        if(
          ...
          return toast(
            "Impossible de supprimer un coureur qui a déjà des temps."
          );
        }
...
function recordLap(){
  ...
  if(
    ...
    /*
      Fin du coureur courant.
      Son temps final est d'abord figé.
    */
    ...
    if(
      ...
      if(next){
        ...
        /*
          ...
          On sélectionne le coureur suivant
          puis on réarme complètement
          le chronomètre.

          Ainsi :
          - les temps du premier restent enregistrés
          - le second repart à 00:00.00
          - DÉPART est à nouveau disponible
        */
      ...
      .map(
        (r,i)=>{
          ...
          return(
            ...
            `<span class="runnerMainMetric">${
              last
                ? fmt(last.cumulativeMs)
                : "Prêt"
            }</span>`+

            `<span class="runnerSubMetric">${
              isDone
                ? "Course terminée"
    ...
    a
      .map(
        x=>
          ...
          `<td>${state.mode==="training"?"—":x.race}</td>`+
          `<td>${x.distance} m</td>`+
          `<td>${fmt(x.lapMs)}</td>`+
